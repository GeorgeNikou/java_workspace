	...
	public static void main(String[] args) {
		...
		while (!exit) {
			...
			if (entry == 2) {
				...
				entry = sc.nextInt();
				x = price.get(entry - 1);
				System.out.println("How many of this item would you like?");// obtaining quantity
				entry2 = sc.nextInt();
				System.out.println("the item you requested: " + items.get(entry - 1)); // displaying the item selected
				System.out.println("the quantity you entered: " + entry2);// displaying the quantity selected
				System.out.println("total: " + "$" + x * entry2);
				...
				if (temp.equalsIgnoreCase("y")) {
					System.out.println("Your grand total is " + grandTotal);// displaying the grand total
					break;
				} else {
					while (true) {
						for (int i = 0; i < items.size(); i++) {
						...
						temp = sc.next();
						if (temp.equalsIgnoreCase("y")) {
							System.out.println("Your grand total is " + (x + y) * entry2);
							break;
						}
					}
